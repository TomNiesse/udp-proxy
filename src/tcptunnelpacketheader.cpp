...
const QByteArray TCPTunnelPacketHeader::encode(const TCPTunnelPacketHeaderType packetType, const size_t connectionId, const QByteArray host, const quint16 port)
{
    ...
    QByteArray encodedConnectionId = QByteArray(connectionIdBytes, sizeof(connectionId));
    ...
    size_t hostSize = host.length();
    char hostSizeBytes[sizeof(hostSize)];
    memcpy(hostSizeBytes, (void*)&hostSize, sizeof(hostSize));
    QByteArray encodedHostSize = QByteArray(hostSizeBytes, sizeof(hostSize));
...
const std::tuple<TCPTunnelPacketHeaderType, size_t, quint16, size_t, QByteArray> TCPTunnelPacketHeader::decode(const QByteArray encodedData)
{
    ...
    // Extract host
    QByteArray host = encodedData.mid(sizeof(TCPTunnelPacketHeaderType)+sizeof(connectionId)+sizeof(port)+sizeof(hostStringSize), hostStringSize);
