...
ProxyServer::ProxyServer(const UDPTunnelConnectionSettings& senderUdpTunnelConnectionSettings, const UDPTunnelConnectionSettings& receiverUdpTunnelConnectionSettings)
{
    // this->hostConnectionManager = std::make_unique<TCPHostConnectionManager>(udpTunnelConnectionSettings);
...
void ProxyServer::communicationThread(const qintptr socketDescriptor, const UDPTunnelConnectionSettings udpTunnelConnectionSettings)
{
    ...
    // ProxyTunnelHost hostConnection(udpTunnelConnectionSettings);
    ...
    // QObject::connect(&hostConnection, &ProxyTunnelHostConnection::clientIsConnected, this, [this, &proxyServerSocket](){
    ...
    // QObject::connect(&hostConnection, &ProxyTunnelHostConnection::receivedData, this, [this, &proxyServerSocket](const QByteArray& data){
    //     proxyServerSocket.write(data);
    // });

    // When the proxy client disconnects, this thread needs to stop
    // QObject::connect(&hostConnection, &ProxyTunnelHostConnection::clientWasDisconnected, &eventLoop, &QEventLoop::quit);
    ...
    // QObject::disconnect(&hostConnection, &ProxyTunnelHostConnection::clientIsConnected, this, nullptr);
    // QObject::disconnect(&hostConnection, &ProxyTunnelHostConnection::receivedData, this, nullptr);
    // QObject::disconnect(&hostConnection, &ProxyTunnelHostConnection::clientWasDisconnected, &eventLoop, &QEventLoop::quit);
    ...
    // hostConnection.quit();
    // QObject::connect(&hostConnection, &ProxyTunnelHostConnection::clientHasQuit, &eventLoop, &QEventLoop::quit);
    eventLoop.exec();
    // QObject::disconnect(&hostConnection, &ProxyTunnelHostConnection::clientHasQuit, &eventLoop, &QEventLoop::quit);
}
