...
//         if(data.length() > 0)
//         {
//             // qDebug() << QDateTime::currentDateTime() << "Start sending data with length:" << data.length() << Qt::flush;
...
//                 egressSocket.waitForBytesWritten();
...
//             // qDebug() << QDateTime::currentDateTime() << "Finished sending data with length:" << data.length() << Qt::flush;
//         }
...
//     acknowledgementPacketHeader.setPacketType(UDPTunnelPacketType::UDP_DATA_ACKNOWLEDGEMENT);
